    ...
    <nav class="bg-gray-800">
        <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <div class="flex items-center justify-between h-16">
                <div class="flex items-center">
                    ...
                    <div class="hidden md:block">
                        <div class="ml-10 flex items-baseline space-x-4">
                            ...
                            <a href="{{ route('news.search') }}" class="block px-3 py-2 rounded-md text-base font-medium text-gray-300 hover:bg-gray-700 hover:text-white {{ Route::currentRouteName() == 'news.search' ? 'bg-gray-900 text-white' : '' }}">Tambahkan Artikel</a>
                            <a href="{{ route('team.index') }}" class="block px-3 py-2 rounded-md text-base font-medium text-gray-300 hover:bg-gray-700 hover:text-white {{ Route::currentRouteName() == 'team.index' ? 'bg-gray-900 text-white' : '' }}">Tambah Team</a>
                ...
                @if (!in_array(Route::currentRouteName(), ['news.search']))
                <div class="hidden md:block">
                    <form action="{{ route('news.searchh') }}" method="GET" class="ml-4 flex items-center md:ml-6">
                        <div class="relative flex">
                            <input type="text" name="search" value="{{ request('search') }}" placeholder="Search..." class="rounded-l-md border border-gray-300 px-4 py-2 focus:border-transparent focus:outline-none focus:ring-2 focus:ring-blue-500 flex-grow">
